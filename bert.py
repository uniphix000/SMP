...
label_list = ['website', 'tvchannel', 'lottery', 'chat', 'match',
...
class MyPro(DataProcessor):
    ...
    def _create_examples(self, dicts, set_type):
        ...
        for i in range(len(dicts)):
            guid = "%s-%s" % (set_type, i)
            text_a = dicts[str(i)]['query']
            label = dicts[str(i)]['label']
            examples.append(
...
def main():
    ...
    parser.add_argument("--data_dir",
                        default='/users4/xhu/SMP/raw_data',
                        type=str,
                        # required = True,
    ...
    predict_box = test(model, processor, args, label_list, tokenizer, device)
    labels = label_list
